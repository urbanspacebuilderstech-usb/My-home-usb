...
# Aug 18 2026 — Delivery photos the SE shoots against a material request.
# `lorry_image_id` is the legacy single-photo form kept for old rows.
_MAT_PHOTO_FIELDS = (
...
async def _enrich_expense_uploads(all_expenses: List[Dict[str, Any]]) -> None:
    """Attach the Site-Engineer uploads to cashbook expense rows, in place.

    The Cashbook → Expense → Transaction Details viewer renders whatever bills
    / photos a row carries. Two families of rows arrive without them:

      • Petty cash. Legacy `recorded_expenses` rows predate the multi-bill
        patch and carry neither `item_bills` nor `bill_file_id`. The Accountant
        approvals queue and `/pm/recorded-expenses` already self-heal via
        site_ops._backfill_item_bills — reuse it rather than reimplement.
      • Material. Rows reach the cashbook as `recorded_expenses` mirrors or
        legacy `material_expenses` docs; only the live `material_requests`
        parent holds the delivery photos (and the USB-MR `request_number` for
        the Expense table's MR Number column).

    Shared by `/accountant/overview` and `/accountant/cashbook-filtered`.
    Both feed the SAME Expense table — CashbookTab falls back to the overview
    payload whenever the (slower, heavier) filtered call hasn't landed — so
    enriching only one of them left the viewer empty depending on which
    response happened to win. Keep them on one code path.
    """
    ...
    for e in rows:
        ...
        if not rid:
            continue
        # `request_id` on a material row is usually a material_expenses
        # expense_id ("mexp_..."), NOT a material_requests.request_id —
        # pay_approval's "material" approval type is keyed by
        # material_expenses, so the mirror it creates stamps that id here.
        # Route those through material_expenses.source_request_id to reach
        # the actual parent.
        (mexp_ids if rid.startswith("mexp_") else mreq_ids).add(rid)

    mexp_to_mreq: Dict[str, str] = {}
    if mexp_ids:
        mexp_docs = await db.material_expenses.find(
            {"$or": [{"expense_id": {"$in": list(mexp_ids)}}, {"material_expense_id": {"$in": list(mexp_ids)}}]},
            {"_id": 0, "expense_id": 1, "material_expense_id": 1, "source_request_id": 1},
        ).to_list(len(mexp_ids))
        for d in mexp_docs:
            mreq_id = d.get("source_request_id")
            if not mreq_id:
                continue
            mreq_ids.add(mreq_id)
            for key in (d.get("expense_id"), d.get("material_expense_id")):
                if key:
                    mexp_to_mreq[key] = mreq_id

    if not mreq_ids:
        return
    parents = await db.material_requests.find(
        {"request_id": {"$in": list(mreq_ids)}},
        {"_id": 0, "request_id": 1, "request_number": 1, **{f: 1 for f in _MAT_PHOTO_FIELDS}},
    ).to_list(len(mreq_ids))
    parent_map = {r["request_id"]: r for r in parents}
    for e in rows:
        rid = e.get("request_id") or e.get("source_request_id")
        if not rid:
            continue
        parent = parent_map.get(mexp_to_mreq.get(rid, rid))
        if not parent:
            continue
...
async def recompute_material_payment_tracking(target_id: str, dry_run: bool = True, user: User = Depends(get_current_user)):
    ...
    when a sibling leg still exists. Already fixed going forward (see the
    recorded_expenses branch above).

    Aug 5 2026 — This originally swept EVERY material_requests /
    material_expenses doc and recomputed paid_amount from whatever
    recorded_expenses rows matched request_id/material_request_id/
    material_expense_id. A dry run surfaced real cross-bill contamination
    in production data — e.g. a ₹9,700 bill picking up a ₹14,950 bill's
    leg too (paid_amount came out to 24650 = 9700+14950) — because those
    linking fields aren't reliably scoped to a single bill for older/
    legacy rows. Applying that broadly would have corrupted far more
    records than it fixed. Restricted to a single explicitly-named
    `target_id` (the material_expenses `expense_id` or material_requests
    `request_id`) so it only ever touches a record you've already
    verified by hand. SUPER_ADMIN only. dry_run=true (default) only
    reports the diff; dry_run=false applies it.
    """
    if user.role != UserRole.SUPER_ADMIN:
    ...
    doc = await db.material_requests.find_one({"request_id": target_id}, {"_id": 0})
    coll = db.material_requests
    match_field = "request_id"
    amt_fields = ["final_price", "estimated_price"]
    if not doc:
        doc = await db.material_expenses.find_one(
            {"$or": [{"expense_id": target_id}, {"material_expense_id": target_id}]}, {"_id": 0}
        )
        coll = db.material_expenses
        match_field = "expense_id" if (doc or {}).get("expense_id") == target_id else "material_expense_id"
        amt_fields = ["final_amount", "estimated_cost"]
    if not doc:
        raise HTTPException(status_code=404, detail=f"No material_requests/material_expenses doc found for id {target_id}")

    legs = await db.recorded_expenses.find(
        {"$or": [{"request_id": target_id}, {"material_request_id": target_id}, {"material_expense_id": target_id}]},
        {"_id": 0, "expense_id": 1, "amount": 1, "status": 1, "payment_method": 1, "created_at": 1},
    ).to_list(200)
    live_legs = [l for l in legs if (l.get("status") or "").lower() not in _EXCL]
    live_paid = sum(float(l.get("amount") or 0) for l in live_legs)
    ...
    result = {
        ...
        "live_legs": live_legs,
        "before": {"paid_amount": doc.get("paid_amount"), "remaining_balance": doc.get("remaining_balance"), "status": doc.get("status")},
    ...
    if not dry_run:
        ...
        await coll.update_one({match_field: target_id}, {"$set": update_fields})
