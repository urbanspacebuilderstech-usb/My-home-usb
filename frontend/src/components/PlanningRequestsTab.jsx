...
import { Package, Users, Wallet, ThumbsUp, ThumbsDown, Loader2, CheckCircle2, AlertCircle, FileText, Calendar, User as UserIcon, Briefcase } from 'lucide-react';

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;
const fmt = (n) => '₹' + (Number(n) || 0).toLocaleString('en-IN');

const TAB_META = {
  material: { label: 'Material', Icon: Package, color: 'blue', activeBg: 'bg-blue-600', pillActive: 'bg-blue-50 text-blue-700 border-blue-300', badgeBg: 'bg-blue-100 text-blue-700' },
  labour:   { label: 'Labour',   Icon: Users,   color: 'amber', activeBg: 'bg-amber-600', pillActive: 'bg-amber-50 text-amber-700 border-amber-300', badgeBg: 'bg-amber-100 text-amber-700' },
  petty:    { label: 'Petty Cash', Icon: Wallet, color: 'emerald', activeBg: 'bg-emerald-600', pillActive: 'bg-emerald-50 text-emerald-700 border-emerald-300', badgeBg: 'bg-emerald-100 text-emerald-700' },
};

// ---- Field accessors per type ----
const getId = (req, type) => type === 'material' ? req.request_id : type === 'labour' ? (req.labour_expense_id || req.expense_id || req.request_id) : req.petty_cash_id;
const getAmount = (req, type) => {
  if (type === 'material') return req.estimated_price || req.estimated_cost || req.final_price || 0;
  if (type === 'labour') return req.total_amount || req.amount || 0;
...
const getTitle = (req, type) => {
  ...
  if (type === 'labour') return req.labour_type || req.contractor_name || req.description || 'Labour';
...
export default function PlanningRequestsTab({ projects = [] }) {
  ...
  const fLabour = useMemo(() => applyFilters(labour), [labour, dateFrom, dateTo, projectFilter]);
  const fPetty = useMemo(() => applyFilters(petty), [petty, dateFrom, dateTo, projectFilter]);

  const counts = { material: fMaterials.length, labour: fLabour.length, petty: fPetty.length };
  const activeList = activeType === 'material' ? fMaterials : activeType === 'labour' ? fLabour : fPetty;
        ...
        {Object.entries(TAB_META).map(([k, meta]) => {
          ...
          return (
            ...
            >
              ...
              <Badge className={`${active ? meta.badgeBg : 'bg-gray-100 text-gray-700'} text-[11px] h-5 px-1.5 min-w-[20px] justify-center font-bold`} data-testid={`planning-pill-${k}-count`}>
                {counts[k]}
              </Badge>
            </button>
          );
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onCancel()}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          {/* Header */}
          <Card className={`border-l-4 ${type === 'material' ? 'border-l-blue-500 bg-blue-50' : type === 'labour' ? 'border-l-amber-500 bg-amber-50' : 'border-l-emerald-500 bg-emerald-50'}`}>
