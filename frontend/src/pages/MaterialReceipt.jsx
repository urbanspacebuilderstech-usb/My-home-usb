...
import { 
  Building2, LogOut, Package, Truck, CheckCircle, XCircle, 
  MapPin, Camera, Phone, Clock, AlertCircle, Navigation,
  QrCode, ShieldCheck, Upload, Eye
} from 'lucide-react';
...
export default function MaterialReceipt() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Separate transit and received orders
  ...
  const getGPSLocation = () => {
    if (!navigator.geolocation) {
      setGpsLocation({ ...gpsLocation, error: 'Geolocation not supported' });
      return;
    }
    
    setGpsLocation({ ...gpsLocation, loading: true, error: null });
  ...
  const handlePhotoUpload = async (event) => {
    ...
    try {
      ...
      setReceiptForm({ ...receiptForm, photo_id: res.data.file_id });
  ...
  const openReceiptDialog = (order) => {
    ...
    setReceiptForm({
      received_qty: order.quantity?.toString() || '',
      otp: '',
      remarks: '',
      photo_id: null
    });
  ...
  const handleSubmitReceipt = async () => {
    ...
    try {
      ...
      if (detail === 'Invalid OTP') {
        ...
        toast.error(detail || 'Failed to submit receipt');
      }
    }
  };
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <Card className="bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
            <CardContent className="p-4">
              ...
              <p className="text-3xl font-bold text-orange-700">{transitOrders.length}</p>
          ...
          <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
            <CardContent className="p-4">
              ...
              <p className="text-3xl font-bold text-green-700">{receivedOrders.length}</p>
          ...
          <div className="space-y-4">
            {transitOrders.length === 0 ? (
              ...
              transitOrders.map((order) => (
                <Card key={order.request_id} className="overflow-hidden">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          Project: {order.project_name || order.project_id}
                        </p>
                      ...
                      <div className="flex flex-col gap-2">
                        <Button 
                          ...
                          data-testid={`receive-btn-${order.request_id}`}
                        >
                        ...
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={() => { setViewOrder(order); setViewDialog(true); }}
                        >
                  ...
                  <div className={`${order.status === 'received_completed' ? 'bg-green-500' : 'bg-yellow-500'} text-white px-4 py-2 flex items-center justify-between`}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">
                        {order.status === 'received_completed' ? 'Fully Received' : 'Partially Received'}
                      </span>
                    </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-4">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-green-600">{order.received_qty} {order.unit}</p>
                      </div>
                      <div>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold">{selectedOrder?.vendor_name}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold">{selectedOrder?.vehicle_number || '-'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-2 border-blue-200 rounded-lg p-4 bg-blue-50">
              <Label className="text-blue-700 font-semibold flex items-center gap-2">
                <QrCode className="h-4 w-4" /> Enter OTP *
              </Label>
              <Input
                ...
                onChange={(e) => setReceiptForm({ ...receiptForm, otp: e.target.value })}
                placeholder="Enter 6-digit OTP"
                className="mt-2 text-center text-2xl tracking-widest font-mono"
              />
              <p className="text-xs text-blue-600 mt-2">
                OTP was sent to Procurement when material was dispatched
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Input
                  ...
                  onChange={(e) => setReceiptForm({ ...receiptForm, received_qty: e.target.value })}
                ...
                {photoPreview ? (
                  <div className="relative">
                    ...
                    <Button
                      ...
                      onClick={() => { setPhotoPreview(null); setReceiptForm({ ...receiptForm, photo_id: null }); }}
                    >
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">{viewOrder.order_id}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold">{viewOrder.material_name}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Quantity</p>
                  <p className="font-semibold">{viewOrder.quantity} {viewOrder.unit}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Vendor</p>
                  <p className="font-semibold">{viewOrder.vendor_name}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Total Amount</p>
                  <p className="font-semibold">₹{viewOrder.total_amount?.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Vehicle Number</p>
                  <p className="font-semibold">{viewOrder.vehicle_number || '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Driver Phone</p>
                  <p className="font-semibold">{viewOrder.driver_phone || '-'}</p>
                </div>
              </div>
