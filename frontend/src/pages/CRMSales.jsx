...
export default function CRMSales() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Deal Close Dialog - Collect Advance */}
      <Dialog open={dealCloseDialog} onOpenChange={setDealCloseDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-green-600">
              <Target className="h-5 w-5" />
              Close Deal & Collect Advance
            </DialogTitle>
            <DialogDescription>
              Confirm deal closure and enter advance payment details
            </DialogDescription>
          </DialogHeader>
          
          {dealCloseREProject && dealCloseLead && (
            <div className="space-y-4">
              {/* Project Summary */}
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                <h4 className="font-semibold text-green-800 mb-2">{dealCloseREProject.project_name}</h4>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <span className="text-gray-600">Client:</span>
                    <span className="ml-1 font-medium">{dealCloseLead.name}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Location:</span>
                    <span className="ml-1 font-medium">{dealCloseREProject.location}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Area:</span>
                    <span className="ml-1 font-medium">{dealCloseREProject.sqft?.toLocaleString()} sqft</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Handover:</span>
                    <span className="ml-1 font-medium">{dealCloseREProject.handover_months || '-'} months</span>
                  </div>
                </div>
                <div className="mt-3 pt-3 border-t border-green-300">
                  <div className="flex justify-between items-center">
                    <span className="text-green-800 font-medium">Total Project Value</span>
                    <span className="text-xl font-bold text-green-700">
                      ₹{(dealCloseREProject.estimated_total || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {advanceAmount && dealCloseREProject?.estimated_total && (
                    <p className="text-xs text-gray-500 mt-1">
                      {((parseFloat(advanceAmount) / dealCloseREProject.estimated_total) * 100).toFixed(1)}% of total value
                    </p>
                  )}
                </div>
                ...
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-blue-800">
                      ₹{(dealCloseREProject.estimated_total - parseFloat(advanceAmount)).toLocaleString()}
                    </span>
                  </div>
                </div>
              )}
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setDealCloseDialog(false)}>
              Cancel
            </Button>
            <Button 
              onClick={handleDealClose}
              className="bg-green-600 hover:bg-green-700"
              disabled={!advanceAmount || parseFloat(advanceAmount) <= 0 || !paymentMode}
              data-testid="confirm-deal-close"
            >
              <CheckCircle className="h-4 w-4 mr-2" />
              Close Deal & Create Project
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
