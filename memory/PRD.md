...
- 2026-06-09: **Payment Schedule — show undated stages (Option A)** — Backend `/api/planning/monthly-schedule` was silently dropping every stage that had no `expected_payment_date`, no `due_date`, and no manual month pin. Audit revealed **304 of 369** total stages were invisible due to this filter. Fix: when `all_months=true` (CRE Board default), include those stages with a new `no_date_set=true` flag. Single-month tabs still skip them (they don't belong to any specific month). Frontend (`CREBoard.jsx`) shows an amber "⚠ No Date Set" badge + "Unscheduled" in the date column. Net effect: fresh login shows 369 stages (full pipeline) instead of 102, identical across every device/browser/incognito.
