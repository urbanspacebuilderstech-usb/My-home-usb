...
- 2026-06-08: **Mr Achyuth Stage-2 milestone amount reconciled** — Stage 2 "Advance payment for Foundation and Plinth Beam Concrete" was stored at ₹956,798 but its 20% slice of project total (₹4,660,113.50) is ₹932,023. Patched `payment_stages.amount` (stage_id `ps_41c15e654adf`) from ₹956,798 → ₹932,023 to match sibling 20% stages (Stages 3 & 4). `amount_received` kept at ₹956,798 since cash was physically collected — the ₹24,775 over-collection now shows as a retained credit on the stage. Script: `backend/scripts/fix_achyuth_stage2_amount.py` (idempotent).
